...
Для работы с сервером используется класс-обёртка над базовым `Api` из стартера. Он реализует композицию: принимает экземпляр `Api` и внутри вызывает его методы `get` и `post`.

**Класс:** `WebLarekApi`

**Конструктор:**

- `constructor(api: IApi)` — принимает объект, реализующий интерфейс `IApi` (имеет методы `get` и `post`).
